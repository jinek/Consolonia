    ...
    public class InputProcessor<T>(IEnumerable<IMatcher<T>> matchers)
    {
        ...
        private void ProcessSingleInput(T input)
        {
            ...
            for (int i = 0; i < Matchers.Length; i++)
            {
                ...
                if (result == AppendResult.NoMatch)
                {
                    ...
                    {
                        _logger.Trace("This matcher was previous top matcher, flushing starting from index {MatcherIndex}", i);
                        FlushStartingFrom(i);
                    }
                    else
                    {
                ...
                if (result != AppendResult.NoMatch)
                {
                    _logger.Trace("Matcher {MatcherIndex} matched, setting current top matcher index to minimum with {CurrentTopMatcherIndex}", i, currentTopMatcherIndex);
                    currentTopMatcherIndex = Math.Min(currentTopMatcherIndex, i);

                    if (i < _previousTopMatcherIndex)
                    {
                        _logger.Trace("Matcher {MatcherIndex} is below previous top matcher index {PreviousTopMatcherIndex}, flushing starting from it", i, _previousTopMatcherIndex);
                ...
                {
                    _logger.Trace("Auto-flush occurred, resetting all matchers from index 0 and setting previous top matcher index to -1");
                    ResetMatchersFrom(0);
            ...
            _logger.Info("Finished processing input: {Input}, current top matcher index: {CurrentTopMatcherIndex}, previous top matcher index: {PreviousTopMatcherIndex}, updating previous to current",
        ...
        private void FlushStartingFrom(int startIndex)
        {
            for (int i = startIndex; i < Matchers.Length; i++)
                ...
                {
                    _logger.Info("Matcher {Matcher} at index {MatcherIndex} flushed successfully, resetting matchers from index {NextMatcherIndex}",
