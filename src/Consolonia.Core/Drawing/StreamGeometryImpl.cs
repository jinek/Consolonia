        ...
        private class StreamGeometryContextImpl : IStreamGeometryContextImpl, IGeometryContext2
        {
            ...
            public void EndFigure(bool isClosed)
            {
                ...
                if (_isFilled)
                {
                    ...
                    var fillRectangles = ProcessRectilinearStrokesToFillRectangles(_geometryImpl._strokes, bound);
                    foreach (var rect in fillRectangles)
                    {
                        _geometryImpl._fills.Add(new Rectangle(rect, _geometryImpl));
                    }
                }
            }
            ...
            private static List<Rect> ProcessRectilinearStrokesToFillRectangles(IReadOnlyList<Line> strokes, Rect bounds)
            {
            ...
            private static bool IsSimpleRectangle(IReadOnlyList<Line> strokes, Rect bounds)
            {
                ...
                var horizontalLines = strokes.Where(s => !s.Vertical).ToList();
                var verticalLines = strokes.Where(s => s.Vertical).ToList();
            ...
            private static List<Rect> HorizontalScanlineFill(IReadOnlyList<Line> strokes, Rect bounds)
            {
                ...
                var verticalEdges = strokes.Where(s => s.Vertical).ToList();
                ...
                for (int y = (int)Math.Ceiling(bounds.Top); y < (int)Math.Floor(bounds.Bottom); y++)
                {
                    ...
                    foreach (var edge in verticalEdges)
                    {
                        ...
                        if (y >= yMin && y < yMax)
                        {
                            intersections.Add(edge.PStart.X); // Vertical line has constant X
                        }
                    }
                    ...
                    for (int i = 0; i < intersections.Count - 1; i += 2)
                    {
                        if (i + 1 < intersections.Count)
                        {
                            ...
                            if (fillWidth > 0)
                            {
                                fillRects.Add(new Rect(fillXStart, y, fillWidth, 1));
                            }
                        }
                    }
                }
