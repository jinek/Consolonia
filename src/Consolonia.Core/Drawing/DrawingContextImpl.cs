        ...
        private void FillRectangleWithBrush(IBrush brush, Rect r)
        {
            ...
            switch (brush)
            {
                case ShadeBrush:
                    ...
                    {
                        Span<Pixel> row = _pixelBuffer.GetRowSpan(y).Slice((int)targetRect.Left, (int)targetRect.Width);
                        for (ushort x = 0; x < (int)targetRect.Width; x++)
                            row[x] = row[x].Shade();
                    }
                ...
                default:
                    for (ushort y = (ushort)targetRect.Top; y < targetRect.Bottom; y++, brushY++)
                    {
                        ...
                        for (ushort x = 0; x < (int)targetRect.Width; x++, brushX++)
                        {
                            Pixel pixelAbove;
                            if (solidColorBrush == null)
                            {
