...
namespace Consolonia.Core.Drawing
{
    internal class DrawingContextImpl : IDrawingContextImpl
    {
        ...
        private readonly PixelBufferSurface _pixelBufferSurface;
        private PixelBufferLayer _pixelBuffer;
        private readonly Matrix _postTransform = Matrix.Identity;
        private Matrix _transform = Matrix.Identity;
        private IConsoleCapabilities _consoleCapabilities;

        public DrawingContextImpl(PixelBufferSurface pixelBufferSurface, PixelBufferLayer layer)
        {
            _consoleCapabilities = AvaloniaLocator.Current.GetService<IConsoleCapabilities>()!;
            _pixelBufferSurface = pixelBufferSurface;
            _pixelBuffer = layer;
            _clipStack.Push(new Rect(_pixelBufferSurface.Position.X, _pixelBufferSurface.Position.Y, _pixelBufferSurface.Size.Width, _pixelBufferSurface.Size.Height));
        }
        ...
        {
            //var surface = AvaloniaLocator.Current.GetService<PixelBufferSurface>();
            //var layer = surface.CreateLayer(new PixelBufferCoordinate(0,0), new PixelBufferSize((ushort)size.Width, (ushort)size.Height));
            //_pixelBuffer = surface.CreateLayer(new PixelBufferCoordinate(0, 0), new PixelBufferSize((ushort)size.Width, (ushort)size.Height));
            if (_pixelBuffer.Size.Width != size.Width || _pixelBuffer.Size.Height != size.Height)
                _pixelBuffer.SetBufferSize(new PixelBufferSize((ushort)size.Width, (ushort)size.Height));    
            return new RenderTarget(_pixelBufferSurface, _pixelBuffer);
        }
        ...
        private void DrawStringInternal(IBrush foreground, string text, IGlyphTypeface typeface, Point origin = new())
        {
            ...
            foreach (string glyph in text.GetGlyphs(_consoleCapabilities.SupportsComplexEmoji))
            {
